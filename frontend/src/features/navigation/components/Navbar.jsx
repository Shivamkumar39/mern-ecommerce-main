...
import * as React from 'react';
import {
  AppBar, Toolbar, IconButton, Typography, Menu, Avatar, Tooltip,
  MenuItem, Badge, Stack, Button, useMediaQuery, useTheme
} from '@mui/material';
import { Link, useNavigate } from 'react-router-dom';
import ShoppingCartOutlinedIcon from '@mui/icons-material/ShoppingCartOutlined';
import FavoriteBorderIcon from '@mui/icons-material/FavoriteBorder';
...
export const Navbar = () => {
  ...
  return (
    <AppBar position="sticky" sx={{ backgroundColor: 'white', boxShadow: 1, color: 'text.primary' }}>
      <Toolbar sx={{ justifyContent: 'space-between', height: '4rem', px: { xs: 2, md: 6 } }}>
        <Typography
          variant="h6"
          component={Link}
          to="/"
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          >
            {loggedInUser?.isAdmin && (
              <MenuItem onClick={handleCloseUserMenu}>
                <Typography component={Link} to="/admin/add-product" sx={{ textDecoration: 'none', color: 'inherit' }}>
                  Add Product
                </Typography>
              </MenuItem>
            )}
            {settings.map((setting) => (
              <MenuItem key={setting.name} onClick={handleCloseUserMenu}>
                <Typography component={Link} to={setting.to} sx={{ textDecoration: 'none', color: 'inherit' }}>
                  {setting.name}
                </Typography>
              </MenuItem>
            ))}
